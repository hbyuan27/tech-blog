...
```
Put it to the right place according to the plugin configuration field "**dockerDirectory**".
...
spring.jpa.hibernate.ddl-auto=create-drop
```
Don't forget to add the "Spring Cloud Connectors" in the pom.xml

Now, go to the project root path, remove the old image to save some diskspace and build a new one:
`sudo docker rmi hbyuan27/demo` (stop and remove containers first)
`sudo mvn clean package docker:build`

 - **Push Demo App Image to Docker Hub**
Login with your Docker Hub account and push the demo app image to your registry:
...
## **TODO - Share Stored Data Between Containers on Cloud Foundry**
  ...
  [1]: https://github.wdf.sap.corp/i075908/setup-linux-ide-with-java-webapp
